...
export interface Track {
    ...
    artist_id: number | null;
    owner_id: number | null; // Added
    owner_name?: string; // Added: uploader name
    artist_name?: string;
    ...
    return {
        ...
        createAlbum(album: Omit<Album, "id" | "created_at" | "artist_name" | "artist_slug">): number {
            ...
            while (attempt < 100) {
                try {
                    const result = db
                        .prepare(
                            `INSERT INTO albums (title, slug, artist_id, owner_id, date, cover_path, genre, description, type, year, download, price, price_usdc, currency, external_links, is_public, visibility, is_release, published_at, published_to_gundb, published_to_ap)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
                        )
                        .run(
                            album.title,
        ...
        promoteToRelease(id: number): void {
            ...
            db.transaction(() => {
                // 1. Insert into releases
                db.prepare(`
                    INSERT OR IGNORE INTO releases (id, title, slug, artist_id, owner_id, date, cover_path, genre, description, type, year, download, price, price_usdc, currency, external_links, visibility, published_at, published_to_gundb, published_to_ap, license, created_at)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                `).run(
                    album.id, album.title, album.slug, album.artist_id, album.owner_id || album.artist_id,
                    album.date, album.cover_path, album.genre, album.description, album.type, album.year,
                    album.download, album.price, album.price_usdc || 0, album.currency, album.external_links, album.visibility,
                    album.published_at, album.published_to_gundb, album.published_to_ap, album.license, album.created_at
                );
                ...
                for (const track of tracks) {
                    db.prepare(`
                        INSERT OR IGNORE INTO release_tracks (release_id, track_id, title, artist_name, track_num, duration, file_path, price, price_usdc, currency, created_at)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    `).run(
                        id, track.id, track.title, track.artist_name, track.track_num, track.duration, 
                        track.file_path, track.price, track.price_usdc || 0, track.currency, track.created_at
                    );
                }
