...
export function createAuthService(db, jwtSecret, adminUser = "admin", adminPass = "admin") {
            ...
            if (!hasUsername || !hasArtistId || !hasRole || !hasGunPub || !hasSubsonic || !hasIsActive) {
                ...
                for (const old of oldAdmins) {
                    ...
                    if (!hasUsername && old.id === 1)
                        username = 'admin';
                    insertStmt.run(old.id, username, old.password_hash, old.created_at, old.updated_at, old.artist_id || null, old.gun_pub || null, old.gun_priv || null, old.subsonic_token || null, old.subsonic_password || null, old.is_active !== undefined ? old.is_active : 1);
                }
                // 4. Drop old table
    ...
    return {
        ...
        async isDefaultPassword(username) {
        ...
        async authenticateUser(username, password, pubKey, proof) {
            console.log(`[AUTH] Attempting login for user: ${username} (hasPubKey: ${!!pubKey}, hasProof: ${!!proof}, hasPassword: ${!!password})`);
            let user = db.prepare("SELECT id, password_hash, artist_id, role, gun_pub, gun_priv, is_active FROM admin WHERE username = ?").get(username);
            ...
            if (pubKey && proof) {
                ...
                if (isValid) {
                    ...
                    if (!user) {
                        ...
                        // Reload user record
                        user = db.prepare("SELECT id, password_hash, artist_id, role, gun_pub, gun_priv, is_active FROM admin WHERE id = ?").get(id);
                    }
            ...
            if (!artistId) {
                ...
                const existingArtist = db.prepare("SELECT id FROM artists WHERE name = ? COLLATE NOCASE").get(username);
                if (existingArtist) {
        ...
        async verifySubsonicToken(username, token, salt) {
        ...
        async verifyGunSignature(message, pubKey, proof) {
            try {
                if (!proof || !pubKey)
                    return false;
                ...
                const verified = await new Promise((resolve) => {
                    try {
                        const result = Gun.SEA.verify(parsedProof, pubKey, (val) => {
                            resolve(val);
                        });
        ...
        async createUser(username, password, artistId, storageQuota = 1024 * 1024 * 1024, pubKey) {
            const hash = await this.hashPassword(password);
        ...
        updateStorageUsed(userId, bytesUsed) {
        ...
        // Mastodon
        async registerMastodonApp(instanceUrl, redirectUri) {
            // Cleanup URL
        ...
        getMastodonAuthUrl(instanceUrl, clientId, redirectUri) {
            const url = new URL(instanceUrl.startsWith("http") ? instanceUrl : `https://${instanceUrl}`);
        ...
        async exchangeMastodonCode(instanceUrl, clientId, clientSecret, redirectUri, code) {
            const url = new URL(instanceUrl.startsWith("http") ? instanceUrl : `https://${instanceUrl}`);
            // Validate SSRF
        ...
        async loginWithMastodon(instanceUrl, redirectUri, code) {
            // 1. Get Client
 ...
 */
export function encryptGunPrivHelper(priv, secret) {
 ...
 */
export function decryptGunPrivHelper(encrypted, secret) {
