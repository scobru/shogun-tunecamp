...
export function createGunDBService(database, server, peers) {
    let gun = null;
    ...
    const cache = {
        sites: { data: [], timestamp: 0 },
        tracks: { data: [], timestamp: 0 },
    ...
    async function setIdentityKeyPair(pair) {
        try {
            if (!pair || !pair.pub || !pair.priv || !pair.epub || !pair.epriv) {
                return false;
            }
            // Basic validation
            if (typeof pair.pub !== 'string' || typeof pair.priv !== 'string')
                return false;
            // Save
            serverPair = pair;
            database.setSetting("gunPair", JSON.stringify(serverPair));
            ...
            if (gun) {
                gun.user().leave();
                gun.user().auth(serverPair, (ack) => {
                    if (!ack.err)
                        console.log(`🔐 Identity Imported & Re-Authenticated: ${serverPair.pub.slice(0, 8)}...`);
                });
            }
            return true;
        }
    ...
    async function registerSite(siteInfo) {
        ...
        const attemptRegistration = async (retryCount = 0) => {
            return new Promise((resolve) => {
                ...
                user.get('tunecamp').get('profile').put(siteRecord, async (ack) => {
                    if (ack.err) {
    ...
    async function registerTracks(siteInfo, album, tracks) {
        ...
        const attemptRegisterTracks = async (retryCount = 0) => {
            const user = gun.user();
            if (!user.is) {
            ...
            const promises = tracks.map(track => {
                ...
                return new Promise((resolve) => {
                    let resolved = false;
                    tracksRef.get(trackSlug).put(trackData, (ack) => {
                        if (resolved)
                            return;
                        resolved = true;
                        if (ack.err)
            ...
            const error = results.find(e => {
                if (!e)
                    return false;
                if (typeof e === 'string' && e.includes("JSON error"))
                    return true;
                if (typeof e === 'object' && e.err === "JSON error!")
                    return true;
                return false;
            });
    ...
    async function unregisterTracks(siteInfo, album) {
        if (!initialized || !gun || !serverPair) {
            return false;
        }
    ...
    async function getDownloadCount(releaseSlug) {
        if (!initialized || !gun)
            return 0;
        return new Promise((resolve) => {
            gun
    ...
    async function incrementDownloadCount(releaseSlug) {
        if (!initialized || !gun)
            return 0;
    ...
    async function getTrackPlayCount(releaseSlug, trackId) {
        if (!initialized || !gun)
            return 0;
        return new Promise((resolve) => {
            gun
    ...
    async function incrementTrackPlayCount(releaseSlug, trackId) {
        if (!initialized || !gun)
            return 0;
    ...
    async function getTrackDownloadCount(releaseSlug, trackId) {
        if (!initialized || !gun)
            return 0;
        return new Promise((resolve) => {
            gun
    ...
    async function incrementTrackDownloadCount(releaseSlug, trackId) {
        if (!initialized || !gun)
            return 0;
    ...
    async function getTrackLikeCount(releaseSlug, trackId) {
        if (!initialized || !gun)
            return 0;
        return new Promise((resolve) => {
            gun.get(REGISTRY_ROOT).get(STATS_NAMESPACE).get("releases").get(releaseSlug)
                .get("tracks").get(trackId).get("likes").once((data) => {
                resolve(data ? parseInt(data, 10) || 0 : 0);
            });
            setTimeout(() => resolve(0), 3000);
        });
    }
    async function incrementTrackLikeCount(releaseSlug, trackId) {
        if (!initialized || !gun)
            return 0;
        ...
        return new Promise((resolve) => {
            gun.get(REGISTRY_ROOT).get(STATS_NAMESPACE).get("releases").get(releaseSlug)
                .get("tracks").get(trackId).get("likes").put(newCount, (ack) => {
                resolve(ack.err ? currentCount : newCount);
            });
            setTimeout(() => resolve(newCount), 2000);
        });
    }
    async function decrementTrackLikeCount(releaseSlug, trackId) {
        if (!initialized || !gun)
            return 0;
        const currentCount = await getTrackLikeCount(releaseSlug, trackId);
    ...
    async function setTrackRating(releaseSlug, trackId, rating) {
        if (!initialized || !gun || !serverPair)
            return;
        return new Promise((resolve) => {
            gun.get(REGISTRY_ROOT).get(STATS_NAMESPACE).get("ratings").get(serverPair.pub)
                .get("releases").get(releaseSlug).get("tracks").get(trackId).put(rating, (ack) => {
                if (ack.err)
                    console.error("Error setting track rating:", ack.err);
                resolve();
            });
            setTimeout(() => resolve(), 2000);
        });
    }
    async function getCommunitySites() {
        if (!initialized || !gun)
            return [];
        // Check cache
        const now = Date.now();
        ...
        return new Promise((resolve) => {
            ...
            gun
                ...
                .get("sites")
                .map()
                .once((directoryData, siteId) => {
                if (!directoryData || siteId === "_")
                    return;
                if (processedIds.has(siteId))
                    return;
    ...
    async function getCommunityTracks() {
        if (!initialized || !gun)
            return [];
        // Check cache
        const now = Date.now();
        ...
        return new Promise((resolve) => {
            ...
            try {
                ...
                console.log("[DEBUG] Error logging serverPair", e);
            }
    ...
    async function registerUser(pubKey, username) {
        if (!initialized || !gun)
            return false;
        const now = Date.now();
        const userRecord = {
    ...
    async function addComment(trackId, data) {
        if (!initialized || !gun)
            return null;
    ...
    async function deleteComment(commentId, pubKey, signature) {
        if (!initialized || !gun)
            return false;
        ...
        if (signature) {
            try {
                const isValid = await Gun.SEA.verify(signature, pubKey);
     ...
     */
    async function getPersistentSiteId(siteInfo) {
        ...
        return new Promise((resolve) => {
            ...
            sitesRef.once((sites) => {
                ...
                siteIds.forEach(siteId => {
                    sitesRef.get(siteId).once(async (siteData) => {
                        try {
    ...
    async function getRemoteSiteSettings(url) {
        try {
