...
export function createImportRoutes() {
    ...
    router.post("/bandcamp", async (req, res) => {
        try {
            const { url } = req.body;
            if (!url || (!url.includes("bandcamp.com") && !url.includes("bcbits.com"))) {
            ...
            if (!tralbumData) {
                 const trackMatch = html.match(/trackinfo:\s*(\[.*?\]),\s*playing_from/s);
                 ...
                 if (tralbumData) {
                     ...
                     if (titleMatch) tralbumData.current = { title: titleMatch[1].split(", by")[0] };
                 }
            }
            ...
            const cover = coverArtId ? `https://f4.bcbits.com/img/a${coverArtId}_10.jpg` : ""; // _10 is original quality
