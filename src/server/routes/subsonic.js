...
export const createSubsonicRouter = (context) => {
    const router = Router();
    ...
    const sendResponse = (res, req, data, status = 'ok') => {
    ...
    const sendError = (res, req, code, message) => {
        const isJson = ensureString(req.query.f) === 'json';
        const status = 'failed';
        const version = '1.16.1';
    ...
    // --- Formatters ---
    const formatTrack = (track, username) => {
    ...
    const formatAlbum = (album, username) => {
    ...
    const formatArtist = (artist, username) => {
    ...
    router.use(async (req, res, next) => {
        ...
        if (!u)
            return sendError(res, req, 10, 'Parameter u is missing');
    ...
    const getMusicDirectory = (req, res) => {
        const username = req.user?.username || 'admin';
        const id = ensureString(req.query.id);
        if (!id)
            return sendError(res, req, 10, 'Missing parameter id');
        ...
        if (id.startsWith('al_')) {
            ...
            const directory = {
                ...
                child: tracks.map((track) => formatTrack(track, username))
            };
            return sendResponse(res, req, { directory });
        }
    ...
    const scrobble = async (req, res) => {
        const { id, submission: subRaw, timestamp } = req.query;
        ...
        for (let i = 0; i < ids.length; i++) {
            ...
            if (!currentId || !currentId.startsWith('tr_'))
                continue;
        ...
        if (!isNaN(albumId)) {
            ...
            if (album) {
                ...
                sendResponse(res, req, {
                    album: {
                        ...albumData,
                        song: tracks.map((track) => formatTrack(track, username))
                    }
                });
                return;
            }
    ...
    const getGenres = (req, res) => {
        ...
        albums.forEach(album => {
            if (album.genre) {
                ...
                genres.forEach(g => {
                    if (!genreMap[g])
                        genreMap[g] = { count: 0, songCount: 0 };
                    genreMap[g].count++;
    ...
    const buildStarredData = (username) => {
        ...
        for (const item of starred) {
            ...
            if (item.item_type === 'artist') {
                ...
                if (artist)
                    artists.push(formatArtist(artist, username));
            }
            else if (item.item_type === 'album') {
                const album = albumMap.get(id);
                if (album)
                    albums.push(formatAlbum(album, username));
            }
            else if (item.item_type === 'track') {
                const track = trackMap.get(id);
                if (track)
                    songs.push(formatTrack(track, username));
            }
        }
    ...
    const getSong = (req, res) => {
        ...
        const track = db.getTrack(trackId);
        if (!track)
            return sendError(res, req, 70, 'Song not found');
        sendResponse(res, req, {
    ...
    const getArtists = (req, res) => {
        ...
        artists.forEach(artist => {
            ...
            if (!indexes[char])
                indexes[char] = [];
            const artistData = formatArtist(artist, username);
            artistData['@albumCount'] = countMap.get(artist.id) || 0;
    ...
    const search = (req, res) => {
        ...
        if (!query)
            return sendError(res, req, 10, 'Missing query parameter');
    ...
    const getUser = (req, res) => {
        ...
        const requestedUser = username || req.user.username;
        sendResponse(res, req, {
            user: {
    ...
    const getPlayQueue = (req, res) => {
        ...
        if (pq.current)
            response.playQueue['@current'] = `tr_${pq.current}`;
        if (pq.positionMs)
            response.playQueue['@position'] = pq.positionMs;
        sendResponse(res, req, response);
    };
    router.get('/getPlayQueue.view', getPlayQueue);
    router.post('/getPlayQueue.view', getPlayQueue);
    // --- Star / Unstar ---
    const incrementGunDBLikeCount = (trackIdStr) => {
        if (!context.gundbService)
            return;
        try {
    ...
    const decrementGunDBLikeCount = (trackIdStr) => {
        if (!context.gundbService)
            return;
        try {
            const trackId = parseInt(trackIdStr.startsWith('tr_') ? trackIdStr.substring(3) : trackIdStr);
    ...
    const setGunDBRating = (trackIdStr, rating) => {
        if (!context.gundbService)
            return;
        try {
            const trackId = parseInt(trackIdStr.startsWith('tr_') ? trackIdStr.substring(3) : trackIdStr);
    ...
    const star = (req, res) => {
        const username = req.user?.username || 'admin';
        const { id, albumId, artistId } = req.query;
        ...
        for (const i of albumIds)
            db.starItem(username, 'album', i);
        for (const i of artistIds)
            db.starItem(username, 'artist', i);
        sendResponse(res, req, {});
    };
    const unstar = (req, res) => {
        ...
        for (const i of albumIds)
            db.unstarItem(username, 'album', i);
        for (const i of artistIds)
            db.unstarItem(username, 'artist', i);
        sendResponse(res, req, {});
    };
    ...
    const setRating = (req, res) => {
        ...
        if (!id)
            return sendError(res, req, 10, 'Missing id');
        const r = parseInt(rating || '');
        if (isNaN(r) || r < 0 || r > 5)
            return sendError(res, req, 10, 'Invalid rating (0-5)');
        let type = 'track';
        if (id.startsWith('al_'))
            type = 'album';
        if (id.startsWith('ar_'))
    ...
    // --- Playlist Management ---
    const createPlaylistEndpoint = (req, res) => {
        const username = req.user?.username || 'admin';
        ...
        if (playlistId) {
            ...
            if (!existing)
                return sendError(res, req, 70, 'Playlist not found');
            if (existing.username !== username && username !== 'admin')
                return sendError(res, req, 50, 'User is not being authorized for the given operation');
        }
        else if (name) {
            // Create new
        ...
        if (songId) {
            ...
            for (const sid of songIds) {
                ...
                if (!isNaN(trackId)) {
                    try {
                        db.addTrackToPlaylist(plId, trackId);
                    }
    ...
    const updatePlaylistEndpoint = (req, res) => {
        const username = req.user?.username || 'admin';
        const playlistId = ensureString(req.query.playlistId);
        const name = ensureString(req.query.name);
        const isPublicStr = ensureString(req.query.public);
        const { songIdToAdd, songIndexToRemove } = req.query;
        if (!playlistId)
            return sendError(res, req, 10, 'Missing playlistId');
    ...
    const deletePlaylistEndpoint = (req, res) => {
        const username = req.user?.username || 'admin';
        const id = ensureString(req.query.id);
        if (!id)
            return sendError(res, req, 10, 'Missing id');
        ...
        if (id) {
            ...
            if (track && track.album_id) {
                ...
                if (album && album.genre) {
                    ...
                    const similarAlbums = allAlbums.filter(a => a.id !== album.id &&
                        a.genre &&
    ...
    const getLyrics = (req, res) => {
        ...
        if (artist && title) {
            ...
            const match = results.tracks.find(t => t.title.toLowerCase() === title.toLowerCase() &&
    ...
    const getOpenSubsonicExtensions = (req, res) => {
        sendResponse(res, req, {
    ...
    const getSongsByGenre = (req, res) => {
        ...
        const offset = ensureString(req.query.offset);
        if (!genre)
            return sendError(res, req, 10, 'Missing genre parameter');
        const limit = parseInt(count || '') || 10;
        const skip = parseInt(offset || '') || 0;
    ...
    // --- Get Shares (stub) ---
    const getShares = (req, res) => sendResponse(res, req, { shares: { share: [] } });
    ...
    const getBookmarksEndpoint = (req, res) => {
        const username = req.user?.username || 'admin';
    ...
    const createBookmarkEndpoint = (req, res) => {
        const username = req.user?.username || 'admin';
        const id = ensureString(req.query.id);
        const position = ensureString(req.query.position);
        const comment = ensureString(req.query.comment);
        if (!id || !position)
            return sendError(res, req, 10, 'Missing id or position');
    ...
    const deleteBookmarkEndpoint = (req, res) => {
        const username = req.user?.username || 'admin';
        const id = ensureString(req.query.id);
        if (!id)
            return sendError(res, req, 10, 'Missing id');
