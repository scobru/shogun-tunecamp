...
export const Sidebar = () => {
  ...
  return (
    <div className="menu p-2 w-20 min-h-full bg-base-100 text-base-content border-r border-white/5 flex flex-col gap-2 pb-28 items-center">
      {/* Brand */}
      <div className="flex items-center justify-center p-2 mb-2 tooltip tooltip-right z-50" data-tip={siteName}>
      ...
      <div className="flex flex-col items-center gap-3 p-2 border-t border-white/5 w-full">
        {isAuthenticated || isAdminAuthenticated ? (
            <div className="flex flex-col items-center gap-3 w-full">
                <div className="avatar placeholder tooltip tooltip-right z-50" data-tip={user?.alias || (isAdminAuthenticated ? 'Admin' : 'User')}>
                ...
                <div className="flex flex-col gap-2 items-center">
                    {isAdminAuthenticated && (
                        <Link to="/admin" className="btn btn-ghost btn-sm btn-circle text-primary tooltip tooltip-right z-50" data-tip="Admin Settings">
                            <Settings size={20} />
                        </Link>
                    )}
                    <button className="btn btn-ghost btn-sm btn-circle opacity-70 hover:opacity-100 hover:text-error tooltip tooltip-right z-50" data-tip="Logout" onClick={handleLogout}>
                        <LogOut size={20} />
                    </button>
                </div>
            </div>
