        ...
        <div className="space-y-6 animate-fade-in">
             <div className="flex items-center justify-between">
                <h1 className="text-3xl font-bold flex items-center gap-3">
                    <User size={32} className="text-black"/> Artists
                </h1>
             ...
             <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6">
                {artists.map(artist => (
                    <Link to={`/artists/${artist.slug || artist.id}`} key={artist.id} className="group text-center">
                        <figure className="aspect-square relative overflow-hidden rounded-none shadow-none mb-4 border-4 border-transparent group-hover:border-black transition-all mx-auto w-full max-w-[200px]">
                            {artist.coverImage ? (
                                <img 
                        ...
                        <h3 className="font-bold truncate text-lg group-hover:text-black transition-colors">{artist.name}</h3>
