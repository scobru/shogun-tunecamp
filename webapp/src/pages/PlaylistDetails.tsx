...
import { Music, Play, MoreHorizontal, Clock, Trash2, Globe, Lock } from 'lucide-react';
...
export const PlaylistDetails = () => {
    ...
    if (loading) return <div className="text-center opacity-50 py-12">Loading playlist...</div>;
    if (!playlist) return null;

    return (
        <div className="space-y-8 animate-fade-in p-6">
            <div className="flex flex-col md:flex-row gap-8 items-end">
                <div className="w-52 h-52 bg-gradient-to-br from-primary to-secondary rounded-2xl shadow-2xl flex items-center justify-center text-6xl text-base-100/50 shrink-0">
                    <Music size={64}/>
                </div>
                
                <div className="flex-1 min-w-0">
                    <div className="uppercase text-xs font-bold tracking-widest opacity-70 mb-2">Playlist</div>
                    <h1 className="text-4xl lg:text-6xl font-black tracking-tighter mb-4 leading-tight">{playlist.name}</h1>
                    <p className="opacity-70 text-lg mb-4 line-clamp-3">{playlist.description}</p>
                    ...
                    <div className="flex flex-wrap items-center gap-4">
                        ...
                        <span className="opacity-50">•</span>
                        <div className="opacity-50">Created {new Date(playlist.createdAt).toLocaleDateString()}</div>
                    </div>
                ...
                <table className="table w-full">
                    ...
                    <tbody>
                        {playlist.tracks && playlist.tracks.map((track, i) => (
                            <tr key={`${track.id}-${i}`} className="hover:bg-white/5 group border-b border-white/5 last:border-0">
                                ...
                                <td>
                                    <div className="flex items-center gap-3">
                                        <div className="avatar rounded w-8 h-8 opacity-80 bg-base-300">
                                            {track.albumId && <img src={API.getAlbumCoverUrl(track.albumId)} loading="lazy"/>}
                                        </div>
                                        <div>
                                    ...
                                    {isAdminAuthenticated && (
                                        <div className="dropdown dropdown-end dropdown-hover opacity-0 group-hover:opacity-100">
                                            ...
                                            <ul tabIndex={0} className="dropdown-content z-[1] menu p-2 shadow bg-base-300 rounded-box w-52 text-sm border border-white/10">
                                                <li><button className="text-error" onClick={() => handleRemoveTrack(track.id)}><Trash2 size={16}/> Remove</button></li>
                                            </ul>
                                        </div>
                                    )}
                                </td>
                        ...
                        {(!playlist.tracks || playlist.tracks.length === 0) && (
                            <tr>
                                <td colSpan={5} className="text-center py-12 opacity-50">No tracks in this playlist yet.</td>
                            </tr>
                        )}
                    </tbody>
                </table>
