    ...
    const loadData = async () => {
      try {
        ...
        validTracks.forEach((t: any) => {
          const title =
            t.federation === "activitypub" ? t.title : t.track.title;
          const artist =
            t.federation === "activitypub"
              ? t.artistName
      ...
      <header className="flex flex-col gap-4 border-b border-white/5 pb-8">
        ...
        {status && (
          <div className="flex flex-wrap gap-4 mt-4">
            <div
              className={`px-3 py-1.5 rounded-full border text-xs font-bold flex items-center gap-2 ${status.gundb?.connected ? "bg-green-500/10 border-green-500/30 text-green-400" : "bg-red-500/10 border-red-500/30 text-red-400"}`}
            >
              <div
                className={`w-2 h-2 rounded-full ${status.gundb?.connected ? "bg-green-400 animate-pulse" : "bg-red-400"}`}
              ></div>
              P2P (GunDB):{" "}
            ...
            <div
              className={`px-3 py-1.5 rounded-full border text-xs font-bold flex items-center gap-2 ${status.activitypub?.enabled ? "bg-blue-500/10 border-blue-500/30 text-blue-400" : "bg-yellow-500/10 border-yellow-500/30 text-yellow-400"}`}
            >
              <div
                className={`w-2 h-2 rounded-full ${status.activitypub?.enabled ? "bg-blue-400" : "bg-yellow-400"}`}
              ></div>
              ACTIVITYPUB:{" "}
        ...
        {filteredTracks.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {filteredTracks.map((item: any, i) => {
              ...
              const uniqueId = isAP
                ? item.slug
                : item.siteUrl + "::" + track.id;
              const isHidden = hiddenTracks.includes(uniqueId);

              if (isHidden && !showHidden) return null;

              const baseUrl =
                !isAP && item.siteUrl ? item.siteUrl.replace(/\/$/, "") : "";
              let coverUrl = isAP
                ? item.coverUrl
                : track.coverImage ||
                  ...
                  <div className="p-3 flex items-center gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="font-bold text-sm truncate pr-2 flex items-center gap-2">
                        ...
                        {isAP ? (
                          <span
                            className="badge badge-accent badge-xs opacity-70"
                            title="Discovered via ActivityPub"
                          >
                            AP
                          </span>
                        ) : (
                          <span
                            className="badge badge-info badge-xs opacity-70"
                            title="Discovered via P2P (GunDB)"
                          >
                            P2P
                          </span>
                        )}
                      ...
                      <div className="text-xs opacity-60 truncate flex items-center gap-1">
                        <span>
                          {isAP ? track.artistName : track.artistName}
                        </span>
          ...
          <div className="text-center py-12 opacity-50 border-2 border-dashed border-white/5 rounded-xl">
            <div className="text-center py-12 opacity-50 border-2 border-dashed border-white/5 rounded-xl">
              ...
              <p className="text-sm mt-2">
                Tracks from other instances will appear here when available.
              </p>
            </div>
          </div>
        )}
            ...
            >
              ...
              <div className="card-body p-4">
                <h3 className="font-bold text-lg group-hover:text-primary transition-colors flex items-center gap-2">
                  {site.name}
                  {site.federation === "activitypub" ? (
                    <span className="badge badge-accent badge-xs">AP</span>
                  ) : (
                    <span className="badge badge-info badge-xs">P2P</span>
                  )}
